...
def make_generators(test_data, t_batch_size, test_labels=None):
    ...
    return test_gen
def make_model(rand_seed, size=20):
    '''
    make initial model
    :param rand_seed:
...
def save_predicted_value_as_csv(predict, test_labels, test_files, class_list):
    ...
    :return: None
    '''
    result = pd.DataFrame(predict, columns=class_list)
...
def TF2_classify(source_dir,output_dir,image_size=224,rand_seed=128):
    ...
    test_gen = make_generators(test_data, t_batch_size, test_labels)

    # init model
    model = make_model(rand_seed, len(class_list))
    # predict
